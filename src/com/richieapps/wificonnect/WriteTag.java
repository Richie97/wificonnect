...
public class WriteTag {
    ...
    private static final String TAG = "sprigganWrite";
    NfcAdapter adapter;
     ...
     * @param name the mimetype for the application
     * @param act the activity that will be using this object
     * @param intent the pending intent that handles all nfc intents
     * @param filter the filters needed for writing to a tag
     */
    public WriteTag(Context context, String name, Activity act, PendingIntent intent, IntentFilter[] filter) {
        ndefExchangeFilters = filter;
        this.activity = act;
        nfcPendingIntent = intent;
...
//        passing a null value disables foreground NDEF push in the specified activity.
     ...
     */
    protected NdefMessage getTextViewAsNdef(TextView view) {
     ...
     */
    protected void setPayloadString() {
     ...
     * ndef messages.
     */
    protected void clearPushMessage() {
        adapter.setNdefPushMessage(null, activity);
    }
     ...
     */
    protected NdefMessage getNdefMessage() {
        return message;
    }
     ...
     */
    protected NdefMessage getMessageFromArray() {
        message = messages[0];
        return message;
    }
     ...
     */
    protected Tag getDetectedTag(Intent intent) {
     ...
     */
    protected boolean isNdefDiscovered(Intent intent) {
     ...
     */
    protected boolean isTagDiscovered(Intent intent) {
     ...
     * to a tag or not.
     */
    protected void disableTagWriteMode() {
        writeMode = false;
        adapter.disableForegroundDispatch(activity);
    }
     ...
     */
    protected void createMimeType(String name) {
     ...
     */
    protected void setMessage(byte[] bytes) {
     ...
     */
    protected boolean writeToTag(NdefMessage aMessage, Tag tag) {
        if (aMessage != null) {
            message = aMessage;
        }
